...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys => {
      console.log('[SW v1.8.3] Caches encontrados:', keys);
      return Promise.all(
        keys
          .filter(k => k !== CACHE_NAME)
          .map(k => {
            console.log('[SW v1.8.3] Deletando cache antigo:', k);
...
self.addEventListener('fetch', event => {
  ...
  // e métodos não cacheáveis (POST, PUT, DELETE, etc.)
