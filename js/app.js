...
function setupAuth() {
  ...
  document.getElementById('btn-login-google').addEventListener('click', async () => {
    const { auth, GoogleAuthProvider, signInWithPopup, db, doc, setDoc, getDoc } = window.__firebase;
    ...
    try {
      ...
      // Verifica se já tem perfil no Firestore, se não, cria
      ...
      // onAuthStateChanged vai cuidar do resto (abrir o app)
    } catch (e) {
      if (e.code !== 'auth/popup-closed-by-user' && e.code !== 'auth/cancelled-popup-request') {
        showError(errEl, 'Erro ao entrar com Google. Tente novamente.');
