...
function init() {
  const { auth, onAuthStateChanged, getRedirectResult, GoogleAuthProvider, db, doc, setDoc, getDoc } = window.__firebase;

  // Processa resultado de redirect do Google (após voltar da página do Google)
  ...
  }).catch(e => console.error('Google redirect result error:', e));
...
function setupAuth() {
  ...
  document.getElementById('btn-login-google').addEventListener('click', () => {
    const { auth, GoogleAuthProvider, signInWithRedirect } = window.__firebase;
    ...
    // O resultado é processado em init() via getRedirectResult após o retorno
  });
